...
('c002','Accesories','computer'),
('c003','MemoryCard','computer'),
('c004','Clothing','TopCategory'),
('c005','Women','Clothing'),
('c006','Shirt','Men'),
('c007','FormalShirt','Shirt'),
('c008','T-Shirt','Men'),
('c009','Electronics','TopCategory'),
('c010','Headphones','Electronics'),
('c011','Men','Clothing'),
('c012','MemoryCard','Electronics');
...
select orders.orderid,orders.date, count(cart.amount * cart.quantity) as amount 
from orders 
inner join cart on 
orders.orderid = cart.orderid 
 where cart.itemshippingStatus = 'shipped'
 and orders.userid='s004' 
group by orders.orderid 
order by orders.date asc;

select parentcategorytitle , count(categoryname) as count
from category
where parentcategorytitle in(
select categoryname 
from category
where parentcategorytitle = 'Topcategory')
group by parentcategorytitle;

delete from products
where products.productname in (select * from(
...
left join orders on
orders.orderid = cart.orderid
where datediff(current_date(),orders.date)>365)) as t)